...
Interactive HTML floor-plan documentation for Satisfactory 1.0 factories. Three factories: **Iron Megafactory**, **Steel & EIB Factory**, **Copper Factory**. No single factory's files are the canonical reference — **the written rules in this document are authoritative**. Built files drift out of date as the project iterates and any factory's floors can fall behind the current conventions between audit passes; treat any existing file as an implementation example to verify, never as a source of truth in its own right.
...
- `iron_megafactory.html`, `steel_eib_factory.html` — main factory pages, both structurally rebuilt onto the current summary-bar/floor-plan-grid/chain-card pattern (see `templates/main-factory-page-template.html`). This note used to call them "legacy single-diagram layout, kept only as historical reference" — that description is stale (confirmed via git history and direct comparison against the template during a style audit) and was corrected here; they are not historical-only artifacts. This does not exempt them from the rule below — verify against the written rules before copying from either, same as any other file.
...
- Refinery is 2W×3D — real in-game footprint is 10m×22m. Its 10m width exceeds a single 8m column, same reasoning as Foundry's 10m width above (rounds up to 2W, not down to 1W — an earlier version of this note got this backwards and sized built Refineries at 1W, which was wrong and has since been corrected). Its 22m length is far longer than any other machine, so it gets its own depth class (3D = 3 rows) rather than reusing Assembler/Foundry's 2D box.
- Manufacturer is 3W×3D — real in-game footprint is 18m×20m. Both dimensions exceed 2 columns/rows (16m), so each rounds up to the next size class: 3W (24m allowance) and 3D (24m allowance).
...
- **Per-machine splitter+merger chain colour exception**: when every machine in a row has its own dedicated input splitter and output merger (see the inline-chain port placement rule below), a single machine's splitter and merger share **one** colour rather than each needing a colour unique across the whole floor — they're always physically adjacent (same machine, same column), so chain position/port labels disambiguate them instead of colour. This keeps per-line colour needs within the 15-swatch palette even when a line has up to 15 machines; colours may repeat between separate lines on the same floor (they're spatially distinct, so no ambiguity). This exception applies only to per-machine chains — LB floors and manifold merge-trees still need every node colour-unique per the general rule above.
- Port zone positions (140×140 block): centre `47,47`; top `47,0`; bottom `47,93`; left `0,47`; right `93,47`.
- **Port placement rule (dedicated LB floor)**: splitter input always top port; merger output always bottom port. Remaining ports assigned by the physical direction of the connected node. This is the *post-flip* convention — supersedes any older file that predates it. This rule assumes the LB floor's fixed vertical flow (input above/below the grid per the External IO strips table) — it does not apply as-is to splitters/mergers placed inline on a production floor (see below).
- **Port placement rule (inline splitter/merger chain on a production floor)**: when every machine in a row has its own dedicated input splitter and output merger (single belt in, single belt out per machine, chained machine-to-machine rather than a manifold with invisible taps), port orientation follows that row's chosen belt-flow direction instead of a fixed top/bottom rule:
  - **RTL row** (input trunk enters from the right): each splitter's input port faces **right**, its continue-the-chain port faces **left** (feeding the next splitter further left). The merger chain runs the opposite way — each merger's output port faces **right**, so the fully-combined output trunk exits on the same (right) side the input entered.
  - **LTR row**: mirror of the above — splitter input faces **left**, continues **right**; merger output faces **left**, exiting on the same (left) side input entered.
  - Declare each row's direction explicitly in that floor's notes (don't leave it implied by pixel position alone) — a reader tracing the diagram needs to know which way the chain flows without reverse-engineering coordinates.
  - This rule doesn't retroactively apply to floors built as manifolds with no visible splitter (the "no splitter needed" pattern used elsewhere) — those floors keep whatever ad hoc terminal/merger styling they already use for the manifold's single combining merger, if any.
...
- 2-way splitter → exact halves; 3-way → exact thirds. An uneven required split (e.g. 30/20 from 50) is not achievable with splitters alone — fix the input rate or design a splitter+merger network. Never fudge this. **Exception — manifold-chain splitters** (a splitter feeding one machine and passing the remainder to the next splitter in a per-machine chain, as opposed to a genuine load-balancer split): label these by actual consumption/remainder at each step (e.g. "→ Machine 30/m" and "→ next splitter 120/m"), not forced-even halves. This reflects real backpressure behavior — the splitter building itself is a plain even split, but once a machine's branch backs up from only pulling what it needs, the surplus routes onward automatically, so the effective flow is consumption-based even though the mechanism is even. The exact-halves rule still applies to genuine load-balancer trees (deliberately calculated even fractions feeding identical downstream demand with no backpressure surplus) — the two are visually and mechanically different (LB floors vs. per-machine chains) and shouldn't be confused.
